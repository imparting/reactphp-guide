...
`$data`参数可能是混合类型，但通常建议它应该是`string`值，
...
`pipe(WritableStreamInterface $dest, array $options = [])` 方法可将此可读源中的所有数据通过管道传输到设定的可写目标源。
...
许多常见的流（例如TCP / IP连接或基于文件的流）可以选择缓冲所有数据，并通过使用底层的`EventLoop`来检查资源何时实际可写来计划将来的刷新。
...
同样，如果流是不可写的(已经处于关闭状态)，它一定不能处理设定的`$data`，并且应该返回`false`，表明调用者应该停止发送数据。

`$data`参数可能是混合类型，但通常建议它应该是一个`string`值，或者使用允许转化为`string`的类型，以最大限度地兼容。
 ...
 * ReactPHP在其整个生态系统中都使用`streams`的概念，所以这个包的许多高级开发者只处理[流使用](#stream-usage)。
   流实例通常是在一些更高级别的组件中创建的，许多开发者实际上从来不需要处理创建流实例的问题。
